 ...
 * @ass1
 */
public class TemperatureSensor extends TimedSensor implements
        HazardSensor, ComfortSensor {
     ...
     *      - If the current temperature reading
     *      (TimedSensor.getCurrentReading()) is between 20 and 26 degrees
     *      (inclusive), the ambient temperature is deemed "comfortable" and
     *      100 should be returned.
     *      - For every degree that the current temperature reading exceeds 26
     *      degrees or goes below 20 degrees, the returned comfort level should
     *      be reduced by 20 from a starting value of 100.
     *      - If the current temperature reading is 15 degrees or below, or is
     *      31 degrees or above, 0 should be returned.
     * @return sensor's current comfort level, 0 to 100
     */
    public int getComfortLevel() {
        if (getCurrentReading() >= 20 && getCurrentReading() <= 26) {
            return 100;
        } else if (getCurrentReading() > 15 && getCurrentReading() < 20) {
