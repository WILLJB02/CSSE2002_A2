 ...
 * @ass1
 */
public class NoiseSensor extends TimedSensor implements
    ...
    /**
     * Returns the current comfort level observed by the sensor, based on the
     * current loudness reading.
     * Subtracts the relative loudness returned by calculateRelativeLoudness()
     * from 1, multiplies the result by 100 (floating point multiplication
     * should be used), then rounds down to the largest integer that is less
     * than or equal to the calculated value.
     *
     * If the result is < 0, 0 is returned. Otherwise, the result is returned.
     *
     * For example, if calculateRelativeLoudness() returns 0.8968 then 10 must
     * be returned. If calculateRelativeLoudness() returns 1.7646 then 0 must
     * be returned.
     * @return level of comfort at sensor location, 0 to 100
     */
    public int getComfortLevel() {
        float comfortLevel = (float) (1 - calculateRelativeLoudness()) * 100;
        if (comfortLevel < 0) {
            return 0;
        } else {
     ...
     * of readings, from 1 to N where N is the number of readings, and
    ...
    public String encode() {
        return String.format("NoiseSensor:%s:%d", super.encode(),
