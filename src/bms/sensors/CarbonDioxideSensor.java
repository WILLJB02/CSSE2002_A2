...
public class CarbonDioxideSensor extends TimedSensor implements HazardSensor,
     ...
     * ideal value.
     */
    private final int variationLimit;
     ...
     * is equal to or more than the variation limit, the comfort is equal to
     * zero percent (0).
     * @return the current comfort level as an integer between 0 and 100
     */
    public int getComfortLevel() {
        int difference = Math.abs((getIdealValue() - getCurrentReading()));
        if (difference >= getVariationLimit()) {
            return 0;
        } else {
            float comfortLevel = (float) ((difference * 100) /
                    getVariationLimit());
            return Math.round(100 - comfortLevel);
        }
    }

    /**
     * Returns true if and only if this CO2 sensor is equal to the other given
     * sensor.
    ...
    public boolean equals(Object obj) {
        if (super.equals(obj)) {
            ...
            return this.getIdealValue() == carbonDioxideSensor.getIdealValue()
                    && this.getVariationLimit() ==
                            carbonDioxideSensor.getVariationLimit();
        } else {
            return false;
        }
    }
     ...
     * Two CO2 sensors that are equal according to equals(Object) should have
     * the same hash code.
     * @return hash code of this sensor
     */
    @Override
    public int hashCode() {
        return super.hashCode() + getVariationLimit() + getIdealValue() +
                getVariationLimit();
    }

    /**
     * Returns the machine-readable string representation of this carbon dioxide
     * sensor.
     * The format of the string to return is:
     *
     *  CarbonDioxideSensor:sensorReading1,sensorReading2,...,sensorReadingN:
     *  frequency:idealValue:varLimit
     *
     * where 'sensorReadingX' is the Xth sensor reading in this sensor's list
     * of readings, from 1 to N where N is the number of readings, 'frequency'
     * is this sensor's update frequency (in minutes), 'idealValue' is this
     * sensor's ideal CO2 concentration, and 'varLimit' is this sensor's
     * variation limit.
    ...
    public String encode() {
        return String.format(
                "CarbonDioxideSensor:%s:%d:%d:%d", super.encode(),
                this.getUpdateFrequency(), this.idealValue,
                this.variationLimit);
    }
}
