...
public class Building implements FireDrill, Encodable {
     ...
     * large enough to be able to accommodate all the rooms on the floor (with
     ...
     * @throws IllegalArgumentException if the given floor does not exist (i.e.
     * getFloorByNumber() is null), or if newWidth < Floor.getMinWidth(), or
     ...
     * decreased dimensions, or if the total size of the current rooms could not
     * be supported by decreased dimensions
     */
    public void renovateFloor(int floorNumber, double newWidth,
        ...
        // checking new dimensions against floor below
        Floor floorBelow = getFloorByNumber(floorNumber - 1);
        if (floorBelow != null && (newWidth > floorBelow.getWidth() ||
                newLength > floorBelow.getLength())) {
            throw new FloorTooSmallException();
        }
        // checking new dimensions against floor above
        Floor floorAbove = getFloorByNumber(floorNumber + 1);
        if (floorAbove != null && (newWidth < floorAbove.getWidth() ||
     ...
     *      - name
     *      - number of floors
     *      - floors (in the same order). Comparison should either directly or
     *      indirectly make use of each floor's equals() method.
    ...
    public boolean equals(Object obj) {
        ...
        boolean floorsEqual = false;
        if (this.getFloors().size() == 0 && building.getFloors().size() == 0) {
            floorsEqual = true;
        }
        //checking if all floors in the building are equal
        for (Floor f : this.getFloors()) {
            floorsEqual = false;
            for (Floor m : building.getFloors()) {
                if (f.equals(m)) {
                    floorsEqual = true;
                    break;
                }
        ...
        return this.getName().equals(building.getName()) && floorsEqual
                && building.getFloors().size() == this.getFloors().size();
    }

    /**
     * Returns the hash code of this building.
     * Two buildings that are equal according to equals(Object) should
     ...
     *  encodedFloorN
     *
     * where 'buildingName' is the building's name, 'numFloors' is the number
     * of floors in the building, and 'encodedFloorX' is the encoded
     * representation of this building's Xth floor, if it exists, in ascending
     * order from 1 to N where N is the number of floors in the building.
     * See Floor.encode().
    ...
    public String encode() {
        ...
        encodedBuilding.append(getName()).append(System.lineSeparator())
                .append(getFloors().size());
        for (Floor f : this.getFloors()) {
            encodedBuilding.append(System.lineSeparator()).append(f.encode());
        }
