...
public class Floor implements FireDrill, Encodable {
     ...
     * according to the old schedule should have its maintenance status set
     * to false.
    ...
    public void createMaintenanceSchedule(List<Room> roomOrder)
        ...
        //checking if rooms appear consecutively
        for (int i = 0; i < roomOrder.size(); i++) {
            int roomNumber = roomOrder.get(i).getRoomNumber();
            Room currentRoom = roomOrder.get(i);
    ...
    public boolean equals(Object obj) {
        ...
        boolean widthEqual = false;
        boolean lengthEqual = false;
        boolean roomsEqual = false;
        //checking if double values are within 0.001 of each other
        if (Math.abs(this.width - floor.getWidth()) <= 0.001) {
            widthEqual = true;
        }
        if (Math.abs(this.length - floor.getLength()) <= 0.001) {
            lengthEqual = true;
        }
        //checking if all rooms on floor are equal
        if (this.getRooms().size() == 0 && floor.getRooms().size() == 0) {
            roomsEqual = true;
        }
        ...
        return this.floorNumber == floor.floorNumber &&
                    widthEqual &&
                    lengthEqual &&
                    this.getRooms().size() == floor.getRooms().size() &&
                    roomsEqual;
    }

    /**
     * Returns the hash code of this floor.
     * Two floors that are equal according to equals(Object) should
    ...
    public int hashCode() {
        ...
        return (int) (total + 85 * floorNumber + Math.round(width) +
                Math.round(length));
    }
     ...
     * 'numRooms' is the number of rooms on this floor, and 'encodedRoomX' is
     * the encoded representation of this floor's Xth room, if it exists, where
     * rooms are appended in insertion order (i.e. the order that rooms were
     * added to this floor by calling Floor.addRoom()). See Room.encode().
     ...
     * System.lineSeparator() should be used to separate lines. There
    ...
    public String encode() {
        String roomWidth = String.format("%.2f", width);
        String roomLength = String.format("%.2f", length);
        StringBuilder encodedFloor = new StringBuilder();
        encodedFloor.append(String.format("%s:%s:%s:%s", floorNumber,
                roomWidth, roomLength, rooms.size()));
        if (maintenanceSchedule != null) {
            encodedFloor.append(
                    String.format(":%s", maintenanceSchedule.encode()));
        }
        for (Room r : this.getRooms()) {
            encodedFloor.append(System.lineSeparator()).append(r.encode());
        }
